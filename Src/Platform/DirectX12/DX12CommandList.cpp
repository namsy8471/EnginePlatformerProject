...
void DX12CommandList::SetVertexBuffer(IBuffer* buffer)
{
	...
	const D3D12_VERTEX_BUFFER_VIEW vertexBufferView = dx12Buffer->GetVertexBufferView();
...
D3D12_RESOURCE_STATES DX12CommandList::TranslateResourceState(ResourceState state)
{
	switch(state)
	{
		...
		case ResourceState::DepthStencil: return D3D12_RESOURCE_STATE_DEPTH_WRITE; // DepthStencil은 Write만 사용
